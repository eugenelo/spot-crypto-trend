...
from momentum.position_generation import nonempty_positions, generate_positions
from momentum.signal_generation import create_trading_signals
from momentum.utils import (
    load_ohlc_to_daily_filtered,
    filter_blacklisted_pairs,
...
def fetch_data(
    ...
    ohlc_combined["timestamp"] = pd.to_datetime(ohlc_combined["timestamp"], unit="ms")
...
def convert_balances_to_cash(kraken):
    ...
    for asset, amount in balance.items():
        if asset != "USD":  # Exclude the base currency
...
def get_pnl(kraken: ccxt.kraken, starting_bankroll: float):
    ...
    pnl = (curr_cash_value - starting_bankroll) / starting_bankroll - 1
...
def get_positions(kraken: ccxt.kraken, df_signals: pd.DataFrame, account_size: float):
    ...
    df_positions = generate_positions(df_signals, params)
...
def main(args):
    ...
    if args.input_path is None:
        ...
        df_ohlc = filter_blacklisted_pairs(df_ohlc)
    else:
        ...
        if args.output_data_freq == "1d":
            df_ohlc = load_ohlc_to_daily_filtered(
                args.input_path, input_freq=args.input_data_freq, tz=tz
            )
    ...
    if args.mode == "data":
            ...
            if args.append_output:
                ...
                if args.output_data_freq == "1d":
                    df_existing_output = load_ohlc_to_daily_filtered(
                        args.output_path, input_freq=args.output_data_freq, tz=pytz.UTC
                    )
                elif args.output_data_freq == "1h":
                ...
                df_ohlc.drop_duplicates(
                    subset=["timestamp", "ticker"], keep="first", inplace=True
                )
        ...
        print(df_nonempty_positions)
        # Output to file
        if args.output_path is not None:
            output_path = Path(args.output_path)
            output_path.parent.mkdir(parents=True, exist_ok=True)
            df_nonempty_positions[
                [
                    "timestamp",
                    "ticker",
                    "scaled_position",
